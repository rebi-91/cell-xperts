...
// ─── Types ────────────────────────────────────────────────────
type MenuKey = "dashboard" | "jobs" | "workers" | "reports" | "settings";
...
const Placeholder: React.FC<{ label: string }> = ({ label }) => (
  <div style={{ padding: 48, textAlign: "center", color: "#6b7280", fontSize: 14, fontFamily: "'DM Sans', sans-serif" }}>
    {label} — coming soon
  </div>
);

// ─── Brand colours ────────────────────────────────────────────
// Dark theme: near-black background with a blue accent (matches the
// dark "Bookings" style reference).
const BRAND = {
  page: "#0d0d0f",
  panel: "#151517",
  panelBorder: "#26262a",
  ...
  primary: "#000000",   // active nav "black" per action-button styling
  accent: "#5b8def",    // blue accent — headings, active nav, primary CTAs
  whatsapp: "#25D366",
};
...
const NAV_ITEMS: { key: MenuKey; label: string; icon: React.ReactNode }[] = [
  { key: "dashboard", label: "Dashboard", icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" style={{display:"block"}}><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg> },
  { key: "jobs",      label: "Jobs Board", icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" style={{display:"block"}}><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="9" y1="6" x2="15" y2="6"/><circle cx="12" cy="15" r="2.5"/></svg> },
  { key: "workers",   label: "Workers",   icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" style={{display:"block"}}><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg> },
  { key: "reports",   label: "Reports",   icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" style={{display:"block"}}><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg> },
  { key: "settings",  label: "Settings",  icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" style={{display:"block"}}><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 010-2.83 2 2 0 012.83 0l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 0 2 2 0 010 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/></svg> },
];
...
const AppSidebar: React.FC<{ active: MenuKey; onSelect: (k: MenuKey) => void }> = ({ active, onSelect }) => (
  <aside style={{
    width: 220, minWidth: 220, maxWidth: 220, height: "100vh",
    background: BRAND.panel, borderRight: `1px solid ${BRAND.panelBorder}`,
    display: "flex", flexDirection: "column", overflowY: "auto",
    flexShrink: 0, boxSizing: "border-box",
  }}>
    {/* Logo — echoes the Cell Xperts mark: bracket "C" + "X" */}
    <div style={{
      height: 56, minHeight: 56, display: "flex", alignItems: "center",
      gap: 8, paddingLeft: 18, borderBottom: `1px solid ${BRAND.panelBorder}`,
      flexShrink: 0, boxSizing: "border-box",
    }}>
      <svg width="26" height="22" viewBox="0 0 26 24" fill="none" stroke={BRAND.accent} strokeWidth="2.5" strokeLinecap="round">
        <path d="M9 4H5v16h4" />
      ...
      <span style={{
        ...
        color: BRAND.textPrimary, letterSpacing: "-0.2px",
      }}>
        Cell <span style={{ color: BRAND.accent }}>Xperts</span>
      </span>
    </div>

    <nav style={{ flex: 1, padding: "8px 10px 16px", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
      {NAV_ITEMS.map(({ key, label, icon }) => {
        const on = active === key;
        return (
          <button
            ...
            style={{
              ...
              width: "100%", boxSizing: "border-box", padding: "9px 12px", marginBottom: 1,
              border: "none", borderRadius: 8,
              background: on ? BRAND.accent : "transparent",
              color: on ? "#fff" : BRAND.textSecondary,
              cursor: "pointer", fontFamily: "'DM Sans', sans-serif",
              fontSize: 13.5, fontWeight: on ? 600 : 400, lineHeight: "20px",
              textAlign: "left", whiteSpace: "nowrap", outline: "none",
              WebkitAppearance: "none", appearance: "none", transition: "background 0.12s, color 0.12s",
            }}
            onMouseEnter={e => { if (!on) (e.currentTarget as HTMLButtonElement).style.background = "#1f1f22"; }}
            onMouseLeave={e => { if (!on) (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}
          >
            <span style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 16, height: 16, minWidth: 16, flexShrink: 0, opacity: on ? 1 : 0.7 }}>
              {icon}
            </span>
    ...
    {/* Footer tagline, matches the card: "Phone Screen Specialists" */}
    <div style={{ padding: "12px 18px", borderTop: `1px solid ${BRAND.panelBorder}`, fontSize: 11, color: BRAND.textSecondary }}>
      Phone Screen Specialists
    </div>
  </aside>
);
...
const Ico = {
  search: <svg width="15" height="15" fill="none" stroke="#9ca3af" strokeWidth="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>,
  bell: <svg width="18" height="18" fill="none" stroke="#d4d4d8" strokeWidth="2" viewBox="0 0 24 24"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg>,
};

const searchPlaceholder: Partial<Record<MenuKey, string>> = {
  dashboard: "Search jobs, customers, workers…",
  jobs:      "Search jobs, customers, workers…",
...
const AppDashboard: React.FC = () => {
  ...
  const [active, setActive] = useState<MenuKey>("dashboard");
  const [userFullName, setUserFullName] = useState("Basak");
  ...
  useEffect(() => {
    ...
    supabase.from("profiles").select("full_name, role").eq("id", session.user.id).single()
      .then(({ data, error }) => {
        ...
        if (!error && data?.role) {
          setUserRole(data.role === "admin" ? "Admin" : "Staff");
        }
      });
  }, [session]);
  ...
  const renderContent = () => {
    switch (active) {
      case "dashboard": return <DashboardPage />;
      case "jobs":       return <JobsPage headerSearch={headerSearch} />;
      case "workers":    return <WorkersPage headerSearch={headerSearch} />;
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "'DM Sans', sans-serif", background: BRAND.page, overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        ...
        {/* Header */}
        <header style={{
          background: BRAND.panel, borderBottom: `1px solid ${BRAND.panelBorder}`,
          padding: "0 24px", height: 56,
          display: "flex", alignItems: "center", gap: 10, flexShrink: 0,
        }}>
          {/* Search */}
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#0f0f11", border: `1px solid ${BRAND.panelBorder}`, borderRadius: 7, padding: "5px 12px", flex: 1, maxWidth: 380 }}>
            ...
            <input
              ...
              onChange={e => setHeaderSearch(e.target.value)}
              placeholder={searchPlaceholder[active] ?? "Search…"}
              style={{ border: "none", background: "transparent", outline: "none", fontSize: 12.5, color: BRAND.textPrimary, width: "100%", fontFamily: "'DM Sans', sans-serif" }}
            />
          </div>
          ...
          {/* User */}
          <div style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", paddingLeft: 10, borderLeft: `1px solid ${BRAND.panelBorder}` }} onClick={() => (window.location.href = "/")}>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: BRAND.textPrimary, lineHeight: 1.3 }}>{userFullName}</div>
              <div style={{ fontSize: 11, color: BRAND.textSecondary }}>{userRole}</div>
            </div>
            <div style={{ width: 36, height: 36, borderRadius: "50%", background: BRAND.accent, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: 13, flexShrink: 0 }}>
              {userInitials}
            </div>
          </div>
        </header>
